...
class _OrderHistoryState extends State<OrderHistory> with SingleTickerProviderStateMixin {
  ...
  getOrderHistory() async {
    ...
    aPiCalls.getOwnedOrder().then((value) async {
      if(value.statusCode == 200 && jsonDecode(value.body)['code'] == 200){
        var data = await jsonDecode(value.body)['data'];
        data.forEach((element) async => await getOrderItems(OrderHistoryModel.fromJson(element).id));
      }else {
  ...
  getOrderItems(id) async {
    aPiCalls.getOrderItems(id).then((value) async {
      if(value.statusCode == 200 && jsonDecode(value.body)['code'] == 200) {
        setState(() => processing = false);
        var data = await jsonDecode(value.body)['data'];
        data.forEach((element) async => setState(() => orderHistoryItemList.add(OrderHistoryItemModel.fromJson(element))));
      }else {
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(item.quantity > 1
                        ? '${item.quantity} Packs N${formatter.format(item.price)}'
                        : '${item.quantity} Pack N${formatter.format(item.price)}',
                      style: blackTextStyle.copyWith(
                          fontSize: 12,
                          fontWeight: FontWeight.w400
                      ),
          ...
          children: [
            ...
            Container(
              ...
              child: MaterialButton(
                child: Text('Login to view Order History',
                    style: blackTextStyle.copyWith(
                        color: Colors.white,
                        fontSize: 14,
